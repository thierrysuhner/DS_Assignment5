...
In total, the leader (now in the minority partition) will lose contact with the majority and therefore, because it can no longer replicate entries to a quorum, step down and tuurn into a follower. The other server in the small partition remains a follower. They may start elections, but as they can't reach a majority, it keeps failing elections (cycles of Candidat -> Follower -> Candidate)
...
Ans: After partitioning, for the majority partition, the only thing that changes is the `partner_node_status_server_node...` for the two nodes that are in the other partition (in our case 4 and 5). They will be set to 0, as these nodes are now unavailable due to partitioning. As this side still has a majority, the leader stays leader, it can still reach a majority. For the two nodes in the minority partition, the leader will eventually be none, the quorum False and also the `partner_node_status_server_node...` which will be 0 for the three nodes in the other partition as before. This is bcause both servers are followers that lose heartbeats from the leader, so they eventually time out. As there is no majority in this partition (only 2/5 nodes), there will be no quorum and the leader will not be existent, as every election process fails due to not enough votes (only 2 out of 5). Therefore, the raft term will continuously increase for every try the nodes make of an election. After reconnection, the original leader of the majority partition will step down as soon as he sees the higher raft term value of a minority partition server (when he receives AppendEntries / RequestVote response containing this hihger term), after that there will be a new leader election for the whole cluster. Therefore, the nodes from the majority cluster will receive a new leader, furthermore all the nodes are now available again (value 2 instead of 0) and all the indices and log_len/last_applied etc. will increase by 1. And of course the raft term gets updated to the higher value of the minority partition servers. For the minority servers, there is now a new leader (the same for everyone in the cluster now), the quorum will be regained to True, all partner nodes are available again (2 instead of 0) and all the indices/log_len etc. will be incremented by 1. Furthermore, all log entries that were committed on the 3-server side replicate to the 2-server side, the cluster returns to stable operation.
...
- After reconnection, in the Secret Lives of Data, the incremented raft term is not authoritative for the minority partition, as they have never won the election. In PySyncObj, the server increments its term on every election timeout. This results in the leader of the majority term stepping down on reconnection in PySyncObj as the servers in the minority partition have higher raft terms. Then a new election starts, etc. In Secret Lives of Data, this doesn't happen, as failed elections do not cause persistent term increases. This differene is because PySyncObj prioritizes simplicity, aggressive consistency guarantees and shortened detection of network failures.
